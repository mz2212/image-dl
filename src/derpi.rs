...
pub fn get_image_links(url: Url, client: reqwest::blocking::Client) -> Vec<Url> {
	...
	for node in dom.find(Class("image-container")) {
		let mut post_body = String::new();
		let mut post_url = url.clone();
		post_url.set_path(node.find(Name("a")).into_selection().first().unwrap().attr("href").unwrap());
		...
		let post_image = post_dom.find(Class("fa-download"))
			.next().unwrap().parent().unwrap();
		
		let image_path = post_image.attr("href").unwrap();
